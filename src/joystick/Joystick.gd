...
export var tap_to_show = false setget set_tap_to_show
export var tap_to_show_container = ""
...
func _ready() -> void:
	if tap_to_show_container == "":
		tap_to_show_container = get_parent()
	map_analog_to_dpad()
...
func need_to_change_pointer(event) -> bool:
	if event is InputEventMouseButton or event is InputEventScreenTouch:
		if tap_to_show:
			return tap_to_show_container.get_global_rect().has_point(Vector2(event.position.x, event.position.y))
		else:
...
func show_at_position(pos) -> void:
	if tap_to_show:
...
func set_tap_to_show(value) -> void:
	if value:
		modulate.a = 0
		if not tap_to_show_container is ViewportContainer:
			assert(
				tap_to_show_container is ViewportContainer, 
